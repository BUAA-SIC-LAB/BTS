...
from tools.det.codet_pbft.BTS_util import check_folder
...
def main(args):
    ...
    training_dataset = V2XSimDet(dataset_roots=[f"{args.data}/agent{i}" for i in agent_idx_range], config=config,
        config_global=config_global, split="train", bound=args.bound, kd_flag=args.kd_flag,
        no_cross_road=args.no_cross_road, )
    training_data_loader = DataLoader(training_dataset, shuffle=True, batch_size=batch_size, num_workers=num_workers,drop_last=True)
    print("Training dataset size:", len(training_dataset))

    logger_root = args.logpath if args.logpath != "" else "logs"
    # 初始化SummaryWriter
    writer = SummaryWriter(log_dir=logger_root)
    ...
    # just for DP
    batch_size = batch_size // torch.cuda.device_count()  # get batch_size for per GPU
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        t = tqdm(training_data_loader,dynamic_ncols=True)
        for sample in t:
        ...
        if need_log:
            ...
            if config.MGDA:
                ...
                save_dict = {"epoch": epoch, "model_state_dict": faf_module.model.state_dict(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--data", default="/{your location}/dataset/CoSwarm-det/train",
    ...
    parser.add_argument("--nworker", default=20, type=int, help="Number of workers")
    parser.add_argument("--lr", default=0.001, type=float, help="Initial learning rate")
    parser.add_argument("--log", action="store_true",default=True, help="Whether to log")
    parser.add_argument("--logpath", default="/{your location}/BTS/coperception/tools/det/runs/train/dp", help="The path to the output log file")
    ...
    parser.add_argument("--auto_resume_path", default="/{your location}/BTS/coperception/tools/det/runs/train/dp", type=str,
