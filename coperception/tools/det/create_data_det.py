...
from coperception.utils.multi_drone_scene_split.parser import parse_scene_files
...
from nuscenes import NuScenes as CoSwarmDet
...
def create_data(coperception_dataset, current_agent, scene_lst, scene_splits, args_savepath):
    channel = "LIDAR_BOTTOM_id_" + str(current_agent)
    total_sample = 0
    ...
    for scene_idx in scene_lst:
        ...
        while channel_flag:
            ...
            (all_pc_teacher, _,) = from_file_multisweep_upperbound_sample_data(
                coperception_dataset, curr_sample_data, return_trans_matrix=False,min_distance=0.1
            )
            ...
            (
                ...
                coperception_dataset, curr_sample_data, return_trans_matrix=False, no_cross_road=True,min_distance=0.1
            )
            ...
            (
                ...
                current_agent, coperception_dataset, curr_sample_data, return_trans_matrix=True,min_distance=0.1,max_num_agent=8
            )
...
def convert_to_dense_bev(seq_data_dict, config):
    ...
    visibility_maps.append(np.zeros(config.map_dims))
    # else:
    #     extents = config.area_extents
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s","--scenes",nargs="+",default=[81],type=int,help="scene ids")
    parser.add_argument(
        "-p",
        "--savepath",
        default="",
    ...
    parser.add_argument(
        "--from_agent", default=1, type=int, help="start from which agent"
    )
    parser.add_argument(
        "--to_agent", default=8, type=int, help="until which agent (index + 1)"
    )
    parser.add_argument(
        "--dataset_version", default='v1.0', type=str, help="[v1.0]"
    )
    args = parser.parse_args()

    dataset_version = args.dataset_version

    scene_lst = args.scenes


    coperception_dataset = CoSwarmDet(version=dataset_version, dataroot=args.root, verbose=True)
    print("Total number of scenes:", len(coperception_dataset.scene))


    scene_files_loc = '../../coperception/utils/multi_drone_scene_split'
    ...
    for current_agent in range(args.from_agent, args.to_agent):
        create_data(coperception_dataset, current_agent, scene_lst, scene_splits, args.savepath)