...
class FaFModule(object):
    def __init__(self, model, teacher, config, optimizer, criterion, kd_flag):
        ...
        if self.MGDA:
            ...
            # self.scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[10,20,30,40,50], gamma=0.5)
            # self.scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[5, 10, 15, 20, 25], gamma=0.5)
    ...
    def step(self, data, batch_size, num_agent):
        ...
        if self.config.flag.startswith("when2com") or self.config.flag.startswith("who2com"):
            if self.config.split == "train":
                ...
                result = self.model(bev_seq, trans_matrices, num_all_agents, batch_size=batch_size, inference=self.config.inference, training=False, )
        else:
            if self.kd_flag:
                result, x_8, x_7, x_6, x_5, fused_layer = self.model(bev_seq, trans_matrices, num_all_agents, batch_size=batch_size)
            else:
    ...
    def step_val(self, data, batch_size, num_agent):
        bev_seq = data["bev_seq"]
        labels = data["labels"]
        reg_targets = data["reg_targets"]
        reg_loss_mask = data["reg_loss_mask"]
    ...
    def get_kd_loss(self, batch_size, data, fused_layer, num_agent, x_5, x_6, x_7):
        if self.kd_flag:
            ...
            kd_loss = kd_weight * (kd_loss_x7 + kd_loss_x6 + kd_loss_x5 + kd_loss_fused_layer)  # kd_loss = kd_weight * (kd_loss_x6 + kd_loss_x5 + kd_loss_fused_layer)  # print(kd_loss)
    ...
    def cls_loss_calculator(self, result, anchors, reg_loss_mask, reg_targets, labels, N, motion_labels=None, motion_mask=None, invert_gt=False, ego_loss_only=False, ego_agent=1, adv_method='pgd'):
        ...
        if adv_method == 'pgd' or adv_method == 'bim':
            ...
            if not ego_loss_only:
                ...
                loss_cls = torch.sum(self.criterion['cls'](result['cls'][ego_agent], labels[ego_agent]))  # print(result['cls'][0].shape,labels[0].shape)  # print("loss cls:", loss_cls)
    ...
    def cal_forward_time(self, data, batch_size, iterations=100):
        ...
        with torch.no_grad():
            for iter in range(iterations):
                ...
                _ = self.model(bev_seq, trans_matrices, num_agent, batch_size=batch_size, pert=pert, no_fuse=no_fuse, unadv_pert=unadv_pert, attacker_list=attacker_list, eps=eps, ego_agent=ego_agent)

                ender.record()
                # sync gpu time
                torch.cuda.synchronize()
                curr_time = starter.elapsed_time(ender)  # cal time
                times[iter] = curr_time  # print(curr_time)
    ...
    def cls_step(self, data, batch_size, invert_gt=False, return_result=False, no_fuse=False, self_result=None, ego_loss_only=False, ego_agent=1, adv_method='pgd'):
        ...
        result = self.model(bev_seq, trans_matrices, num_agent, batch_size=batch_size, pert=pert, no_fuse=no_fuse, unadv_pert=unadv_pert, attacker_list=attacker_list, eps=eps, ego_agent=ego_agent)
    ...
    def predict_all(self, data, batch_size, validation=True, num_agent=5):
        ...
        if self.config.flag.startswith("when2com") or self.config.flag.startswith("who2com"):
            ...
            # starter, ender = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
            # times = torch.zeros(100)     # array for saving inference time
            # with torch.no_grad():
            ...
            # print("Inference time: {:.6f} ms, FPS: {} ".format(mean_time, 1000 / mean_time))
            # import csv
            # csv_file = "robosac.csv"
            # write_header = not os.path.exists(csv_file)
            # # 用于记录通信开销和通信时间
            # with open(csv_file, "a", newline="") as f:
            #     writer = csv.writer(f)
            #     if write_header:
            #         writer.writerow(["robosac_mean_time"])
            #     writer.writerow([mean_time])
            # st = time.time()
            result = self.model(bev_seq, trans_matrices, num_all_agents, batch_size=batch_size, pert=pert, no_fuse=no_fuse, unadv_pert=unadv_pert, collab_agent_list=collab_agent_list,
                trial_agent_id=trial_agent_id, attacker_list=attacker_list, eps=eps, ego_agent=ego_agent)
            # print(f"result in {time.time() - st:4f} seconds")
        N = bev_seq.shape[0]
        ...
        for k in range(num_agent):
            ...
            # print(f"bev_seq: {bev_seq.shape} {bev_seq.device}")
        ...
        # print(f"predict {time.time() - st:4f} secondsx")
    ...
    def predict_all_in_ego(self, data, batch_size, num_agent=5):
        bev_seq = data["bev_seq"]
        # vis_maps = data["vis_maps"]
        trans_matrices = data["trans_matrices"]
        num_all_agents = data["num_agent"]
        ...
        if self.config.flag.startswith("when2com") or self.config.flag.startswith("who2com"):
            ...
            # print("Inference time: {:.6f} ms, FPS: {} ".format(mean_time, 1000 / mean_time))
            # import csv
            # csv_file = "DAD_forward.csv"
            # write_header = not os.path.exists(csv_file)
            # # 用于记录通信开销和通信时间
            # with open(csv_file, "a", newline="") as f:
            #     writer = csv.writer(f)
            #     if write_header:
            #         writer.writerow(["DAD_mean_time"])
            #     writer.writerow([mean_time])
        ...
        seq_results = [[] for _ in range(num_agent)]

        for k in range(num_agent):
            bev_seq = torch.unsqueeze(data["bev_seq"][ego_agent, :, :, :, :], 0)
            if torch.nonzero(bev_seq).shape[0] == 0:
                seq_results[k] = []
            else:
                batch_box_preds = torch.unsqueeze(result["loc"][k, :, :, :, :, :], 0)
                batch_cls_preds = torch.unsqueeze(result["cls"][k, :, :], 0)
                anchors = torch.unsqueeze(data["anchors"][ego_agent, :, :, :, :], 0)
                batch_motion_preds = None
    ...
    def predict_all_with_box_com(self, data, trans_matrices_map, validation=True):
        ...
        for k in range(NUM_AGENT):
            ...
            if torch.nonzero(bev_seq).shape[0] == 0:
                ...
                (global_boxes_af_localnms[k], box_scores_af_localnms[k],) = apply_box_global_transform_af_localnms(trans_matrices_map[k], class_selected, box_scores_pred_cls)
                # print(cls_preds[k].shape, box_scores_af_localnms[k].shape)

                forward_message_size = forward_message_size + 256 * 256 * 6 * 4 * 2
                forward_message_size_two_nms = (forward_message_size_two_nms + global_boxes_af_localnms[k].shape[0] * 4 * 2)
        ...
        for k in range(num_sensor):
            local_results_af_local_nms[k], ms = apply_box_local_transform(class_selected_global_af_local_nms, trans_matrices_map[k])
