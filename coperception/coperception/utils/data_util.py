...
def apply_pose_noise(pose_noise: float, trans_matrices, target_pairs=None, noise_std=0.02):
    """Apply pose noise to specific transformation matrices (non-destructive)

    Draw noise from normal distribution with mean of pose noise and std of noise_std,
    apply to displacements of the transformation matrices in specified (batch, j, i) indices.

    Args:
        pose_noise (float): the mean of pose noise drawn from normal distribution
        trans_matrices (tensor): the transformation matrices with shape (batch, agents, agents, 4, 4)
        target_pairs (list, optional): list of (batch_idx, j, i) tuples to apply noise
        noise_std (float): standard deviation of the noise distribution

    Returns:
        tensor: new transformation matrices with noise applied
    """
    # 创建输入矩阵的深拷贝
    noisy_matrices = trans_matrices.clone()

    # 如果未指定目标对，则对所有矩阵添加噪声
    if target_pairs is None:
        batch_size, num_agents = trans_matrices.shape[:2]
        target_pairs = [(b, j, i) for b in range(batch_size)
                        for j in range(num_agents)
                        for i in range(num_agents)]

    # 为每个目标对添加噪声
    for b, j, i in target_pairs:
        # 检查索引有效性
        if (b < 0 or b >= trans_matrices.shape[0] or
                j < 0 or j >= trans_matrices.shape[1] or
                i < 0 or i >= trans_matrices.shape[2]):
            raise IndexError(f"Index ({b}, {j}, {i}) out of range")

        # 生成噪声矩阵并应用到位移部分
        noise = torch.randn(3, device=trans_matrices.device) * noise_std + pose_noise
        noisy_matrices[b, j, i, :3, 3] += noise  # 仅修改位移部分 [x, y, z]
...
def add_binary_noise(padded_voxel_points, target_nums, noise_prob=0.05):
    """
    为体素化点云的指定num维度添加二值噪声（保持0/1值）

    Args:
        padded_voxel_points (torch.Tensor): 体素化点云，形状为 (num, 1, 256, 256, 13)
        target_nums (list): 需要添加噪声的num索引列表（如 [0, 2, 5]）
        noise_prob (float): 每个元素被翻转的概率（默认0.05，表示5%的元素会被翻转）

    Returns:
        torch.Tensor: 添加噪声后的体素化点云
    """
    # 创建输入数据的深拷贝，避免修改原始数据
    noisy_voxels = padded_voxel_points.clone()
    # 对每个指定的num维度添加噪声
    for num_idx in target_nums:
        # 检查索引是否合法
        if num_idx < 0 or num_idx >= padded_voxel_points.shape[0]:
            raise IndexError(f"num索引 {num_idx} 超出范围（总样本数为 {padded_voxel_points.shape[0]}）")
        # 生成与当前num维度数据同形状的随机概率矩阵
        random_mask = torch.zeros_like(padded_voxel_points[num_idx], dtype=torch.float32)
        # 找出所有需要翻转的元素（概率小于noise_prob的位置）
        # flip_mask = random_mask < noise_prob
        # 对这些元素执行逻辑非操作（0→1，1→0）
        # noisy_voxels[num_idx][flip_mask] = 1 - noisy_voxels[num_idx][flip_mask]
        noisy_voxels[num_idx] = random_mask

    return noisy_voxels

# added by liuzhenghao
def apply_circular_mask(data, batch_size, radius, num_agent=8, is_circular=False):
    data = data.clone()
    batch, seq, z, h, w = data.shape
    # print(batch,num_agent,batch_size)
    assert batch == num_agent * batch_size
    center_h, center_w = h // 2, w // 2
    # 创建一个全零的掩码
    mask = torch.zeros((h, w), dtype=torch.float32, device=data.device)
    if is_circular:
        # 生成圆形掩码
        y_grid, x_grid = torch.meshgrid(torch.arange(h, device=data.device),
        ...
        # 生成矩形掩码
        start_h = max(0, center_h - radius)
        end_h = min(h, center_h + radius)
        start_w = max(0, center_w - radius)
        end_w = min(w, center_w + radius)
        mask[start_h:end_h, start_w:end_w] = 1.0

    # 扩展掩码维度以匹配 data 维度
    mask = mask.unsqueeze(0).unsqueeze(0).unsqueeze(0).repeat(batch, seq, z, 1, 1)
    # 前 batch_size 个样本不应用掩码(RSU)
    mask[:batch_size] = 1.0
    # ego 1 mask
    # mask[batch_size:2*batch_size] = 0.0
