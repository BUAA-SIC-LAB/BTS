...
def apply_nms_det(batch_box_preds, batch_cls_preds, anchors, code_type, config, batch_motion=None):
    ...
    batch_id = 0
    for box_preds, cls_preds, anchors in zip(batch_box_preds, batch_cls_preds, batch_anchors):
        # N  * (W X H) * T * decoded_loc_dim(6)
        ...
        for i in range(boxes_for_nms.shape[1]):
            if code_type[0] == "f":
                if config.pred_type == "motion":
                    ...
                    decoded_boxes = (bev_box_decode_torch(boxes_for_nms[:, i], anchors).cpu().detach().numpy())
                # print(w_id,decoded_boxes[w_id])
                box_pred_corners = center_to_corner_box2d(decoded_boxes[:, :2], decoded_boxes[:, 2:4], decoded_boxes[:, 4:])  # corners: [N, 4, 2]
            elif code_type[0] == "c":
        ...
        for i in range(total_scores.shape[1]):
            ...
            cls_pred_first_nms = cls_preds[selected_idx, :]  # break
        predictions_dicts.append(class_selected)
...
def visualization(config, data, box_colors, box_color_map, apply_late_fusion, savename=None):
    ...
    #     # "lowerbound" : (26, 67, 32), #green
    #     "consensus": (24, 55, 114, 161, 174, 201), #blue
    #     # "original_fusion": (24, 55, 114, 161, 174, 201), #blue
    #     "original_fusion": (120, 33, 45, 200, 56, 77), #red
    #     # "attacked_fusion": (64, 10, 90, 219, 151, 254), #violet
    #     "BTS_fusion": (64, 10, 90, 219, 151, 254), #violet
    #     "attacked_fusion": (60, 65, 70, 141, 153, 153), #grey
    #     # "upperbound": (120, 33, 45, 200, 56, 77) #red
    #     # "upperbound": (128, 105, 19, 255, 188, 82), # orange
    # }
    # ego, other
    vis_color = {
        "LB":(26, 67, 32), # green
        "UB":(109,72,3, 251,204,116), # orange
        "IUB":(62,8,7, 246,152,161), # red
        "ND":(60, 65, 70, 160,167,174), #grey
        "consensus":(24, 55, 114, 161, 174, 201), #blue
        "BTS":(55,14,78, 210,159,238),# purple
        "PASAC": (51,51,0, 204,204,0),  # yellow
    }

    # bg_color = {
    #     "ego_only" : (248, 253, 253),
    #     # "lowerbound" : (248, 253, 253),
    #     "consensus": (247, 251, 247),
    #     "original_fusion": (255, 250, 247),
    #     "BTS_fusion": (254, 247, 244),
    #     "attacked_fusion": (245, 250, 250),
    #     # "upperbound": (255, 250, 247)
    # }
    bg_color = {
        "LB" : (230,245,232), # green
        "UB": (255,249,240),  # orange
        "IUB" : (253,239,236), # red
        "ND": (229,231,233),  # grey
        "consensus": (248, 253, 253), # blue
        "BTS": (245,234,251), # purple，
        "PASAC": (255,255,229), # yellow
    }
    if vis_tag == "LB":
    ...
    elif vis_tag in ["UB", "IUB", "ND", "consensus", "BTS", "PASAC"]:
