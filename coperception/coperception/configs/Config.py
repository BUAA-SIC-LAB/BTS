...
class Config(object):
    ...
    def __init__(
        ...
        # added by Zhenghao Liu
        self.map_range = 32.0
        self.area_extents = (
            # np.array([[-32.0, 32.0], [-32.0, 32.0], [-17.5, -11.0]]) # 15m
            np.array([[-self.map_range, self.map_range], [-self.map_range, self.map_range], [-16.0, -11.0]])  # 15m
            if is_cross_road
            else np.array([[-self.map_range, self.map_range], [-self.map_range, self.map_range], [-41.0, -36.0]])  # 40m
            # else np.array([[-32.0, 32.0], [-32.0, 32.0], [-42.5, -36.0]]) # 40m
        )
